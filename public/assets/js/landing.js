document.addEventListener("DOMContentLoaded", function () {
  ...
  const showTooltip = (x, y, html) => {
    ...
    tooltip.style.left =
      (x + 14 + tw > window.innerWidth ? x - tw - 14 : x + 14) + "px";
    tooltip.style.top =
      (y + 14 + th > window.innerHeight ? y - th - 14 : y + 14) + "px";
  };
  const hideTooltip = () => {
  ...
  const drawBar = (canvas, a, b) => {
    ...
    const baseY = h - padB,
      usableH = h - padT - padB;
    const FILL = 0.85;
    const bh1 = usableH * FILL,
      bh2 = usableH * FILL;
    const barW = Math.max(36, Math.min(80, Math.floor(w * 0.16)));
    const plotW = w - padL - padR;
    const x1 = padL + Math.floor(plotW * 0.18),
      x2 = padL + Math.floor(plotW * 0.57);
    const TICKS = 4;
    ...
    const drawLeftAxis = () => {
      ...
      for (let i = 0; i <= TICKS; i++) {
        const frac = i / TICKS,
          y = baseY - frac * usableH * FILL,
          val = Math.round(frac * a);
    ...
    const drawRightAxis = () => {
      ctx.strokeStyle = "rgba(214,173,0,.7)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(w - padR, padT);
      ctx.lineTo(w - padR, baseY);
      ctx.stroke();
      ctx.fillStyle = "rgba(160,120,0,.9)";
      ctx.font = "9px Poppins, Arial";
      ctx.textAlign = "left";
      for (let i = 0; i <= TICKS; i++) {
        const frac = i / TICKS,
          y = baseY - frac * usableH * FILL,
          val = Math.round(frac * b);
        ctx.strokeStyle = "rgba(214,173,0,.7)";
        ctx.beginPath();
        ctx.moveTo(w - padR, y);
    ...
    // Bars
    ctx.fillStyle = "rgba(99,102,241,.65)";
    ctx.fillRect(x1, baseY - bh1, barW, bh1);
    ctx.fillStyle = "rgba(245,196,0,.80)";
    ctx.fillRect(x2, baseY - bh2, barW, bh2);
    ...
    ctx.fillText("Total LPJU", x1 + barW / 2, baseY + 14);
    ctx.fillText("Total Daya", x2 + barW / 2, baseY + 14);
    ctx.font = "italic 8px Poppins, Arial";
    ctx.fillStyle = "rgba(99,102,241,.7)";
    ctx.textAlign = "left";
    ctx.fillText("(unit)", padL, padT - 8);
    ctx.fillStyle = "rgba(160,120,0,.7)";
    ctx.textAlign = "right";
    ctx.fillText("(Watt)", w - padR, padT - 8);
  ...
  const drawPie = (
    ...
    const cx = w / 2,
      cy = h / 2,
      r = outerLabels ? Math.min(w, h) * 0.24 : Math.min(w, h) * 0.4;
    const colors = [
      "#0b57b7",
      "#f5c400",
      "#f59e0b",
    ...
    ctx.fillStyle = "#fff";
    ctx.fill(); // Match background if not white
    ctx.fillStyle = "#111827";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ...
    ctx.fillText(String(centerLabel ?? total), cx, cy + 10);
    ...
    if (outerLabels && slices.length) {
      ...
      const annotated = slices.map((s) => {
        const mid = (s.start + s.end) / 2,
          cAngle = mid - Math.PI / 2;
        const rimX = cx + Math.cos(cAngle) * r,
          rimY = cy + Math.sin(cAngle) * r;
        const elbX = cx + Math.cos(cAngle) * (r + SPOKE),
          elbY = cy + Math.sin(cAngle) * (r + SPOKE);
        const isRight = Math.cos(cAngle) >= 0;
        const shelfX = isRight ? elbX + SHELF : elbX - SHELF;
        const availW = isRight
          ? Math.max(30, w - shelfX - 8)
          : Math.max(30, shelfX - 8);
      ...
      const rightGroup = annotated
        .filter((s) => s.isRight)
        .sort((a, b) => a.rimY - b.rimY);
      const leftGroup = annotated
        .filter((s) => !s.isRight)
        .sort((a, b) => a.rimY - b.rimY);
      ...
      const assignY = (group) => {
        ...
        if (n === 0) return;
        if (n === 1) {
          group[0].labelY = cy - group[0].blockH / 2;
          return;
        }
        const totalH =
          group.reduce((sum, s) => sum + s.blockH, 0) + (n - 1) * MIN_GAP;
        let startY = Math.max(VPAD, (h - totalH) / 2);
        if (startY + totalH > h - VPAD) startY = h - VPAD - totalH;
        group.forEach((s) => {
          s.labelY = startY;
          startY += s.blockH + MIN_GAP;
        });
      };
      assignY(rightGroup);
      assignY(leftGroup);
      ...
      const drawCallout = (s) => {
        ...
        ctx.globalAlpha = 0.8;
        ctx.beginPath();
        ctx.moveTo(s.rimX, s.rimY);
        ctx.lineTo(s.elbX, s.elbY);
        ctx.lineTo(s.shelfX, anchorY);
        ctx.stroke();

        ctx.fillStyle = s.color;
        ctx.globalAlpha = 1;
        ctx.beginPath();
        ctx.arc(s.rimX, s.rimY, 2.5, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.font = `700 ${FONT_SZ}px Poppins, Arial`;
        ctx.fillStyle = "#111827";
        s.nameLines.forEach((line, i) => {
          ctx.fillText(line, textX, s.labelY + i * LINE_H);
        });

        ctx.font = `${FONT_SZ}px Poppins, Arial`;
        ctx.fillStyle = s.color;
        ctx.fillText(
          s.value.toLocaleString("id-ID") + " (" + s.pct + "%)",
          textX,
          s.labelY + s.nameLines.length * LINE_H + TEXT_GAP,
        );
        ctx.restore();
      };

      rightGroup.forEach(drawCallout);
      leftGroup.forEach(drawCallout);
    }
  ...
  const attachBarTooltip = (canvas, bars) => {
    if (canvas._barMove)
      canvas.removeEventListener("mousemove", canvas._barMove);
    if (canvas._barLeave)
      canvas.removeEventListener("mouseleave", canvas._barLeave);
    canvas._barMove = (e) => {
      const rect = canvas.getBoundingClientRect();
      const cssMx = e.clientX - rect.left,
        csMy = e.clientY - rect.top;
      const hit = bars.find(
        (b) =>
          cssMx >= b.x &&
          cssMx <= b.x + b.w &&
    ...
    canvas.addEventListener("mousemove", canvas._barMove);
    canvas.addEventListener("mouseleave", canvas._barLeave);
  };

  const attachPieTooltip = (canvas, slices, cx, cy, innerR, outerR) => {
    if (canvas._pieMove)
      canvas.removeEventListener("mousemove", canvas._pieMove);
    if (canvas._pieLeave)
      canvas.removeEventListener("mouseleave", canvas._pieLeave);
    canvas._pieMove = (e) => {
      const rect = canvas.getBoundingClientRect(),
      ...
      showTooltip(
        e.clientX,
        e.clientY,
        `<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${hit.color};margin-right:6px;"></span><strong>${hit.label}</strong><br>Jumlah: <strong>${hit.value}</strong><br>Persentase: <strong>${hit.pct}%</strong>`,
      );
    };
    canvas._pieLeave = () => hideTooltip();
    canvas.addEventListener("mousemove", canvas._pieMove);
    canvas.addEventListener("mouseleave", canvas._pieLeave);
  };
...
(function () {
  ...
  function statusBadgeColored(status) {
    ...
    if (!raw) {
      ...
      // recognize various in-progress variants; 'sedang dikerjakan' gets yellow
      key.includes("sedang dikerjakan")
    ) {
      cls = "bg-warning";
    } else if (
      key.includes("sedang diperbaiki") ||
      key.includes("in progress") ||
      key.includes("pending") ||
      key.includes("belum dikerjakan")
    ) {
      cls = "bg-primary";
    } else {
      cls = "bg-secondary";
    ...
    // Use dark text for warning (yellow) badges, white text otherwise
    const textColor = cls === "bg-warning" ? "text-dark" : "text-white";
    return `<span class="badge ${cls} ${textColor}">${escapeHTML(label)}</span>`;
  }
  ...
  async function cekStatus() {
    ...
    if (!nomor) {
      showInModal(
        '<div class="alert alert-warning mb-0">Nomor tiket harus diisi</div>',
      );
      return;
    }
    ...
    const cfg = window.__DASHBOARD_DATA__ || {};
    const apiBaseUrl =
      cfg.apiCekStatusUrl || window.location.origin + "/api/cek-status";
    ...
    try {
                    ...
                    <div class="card border-0 shadow-sm">
                      <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-3 border-bottom pb-2">
                          <div class="mb-0">No. Tiket: <span class="text-primary">${escapeHTML(d.nomor_tiket || nomor)}</span></div>
      ...
      showInModal(
        '<div class="alert alert-danger mb-0"><i class="fa-solid fa-triangle-exclamation me-2"></i> Terjadi kesalahan jaringan. Coba lagi.</div>',
      );
