...
if (typeof Html5Qrcode === "undefined") {
  console.error(
    "CRITICAL ERROR: HTML5-QRCode library is not loaded. Please check the script tags in your HTML.",
  );
  document.addEventListener("DOMContentLoaded", function () {
    var el = document.getElementById("reader");
    if (el)
      el.innerHTML =
        '<div class="alert alert-danger">Sistem pemindai gagal dimuat. Library HTML5-QRCode tidak ditemukan. Muat ulang halaman atau periksa koneksi internet.</div>';
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function describeError(err) {
    ...
    if (name === "NotAllowedError") {
      ...
      hint =
        "Kamera sedang digunakan aplikasi lain atau hardware error. Tutup aplikasi kamera lain dan coba lagi.";
    } else if (name === "OverconstrainedError") {
      hint =
        "Constraint kamera tidak didukung perangkat ini (misal: facingMode environment). Akan dicoba fallback.";
    } else if (name === "AbortError") {
      hint = "Permintaan kamera dibatalkan. Coba muat ulang halaman.";
    } else if (name === "TypeError") {
      hint =
        "Kemungkinan halaman tidak diakses via HTTPS/localhost sehingga navigator.mediaDevices tidak tersedia.";
    }
  ...
  function handleDecodedText(decodedText) {
    ...
    try {
      // Direct URL fallback
      if (
        decodedText.includes("/pju/detail") ||
        decodedText.includes("/kwh/detail")
      ) {
      ...
      if (decodedText.includes("wa.me")) {
        ...
        if (isPJU) {
          targetUrl =
            baseUrl + "/pju/detail?id=" + encodeURIComponent(secureId);
        } else if (isKWH) {
          targetUrl =
            baseUrl + "/kwh/detail?id=" + encodeURIComponent(secureId);
        }
        ...
        if (targetUrl !== "") {
          ...
          if (readerEl) {
            readerEl.innerHTML =
              '<div style="padding: 30px; text-align: center; color: #198754;">' +
  ...
  function startScanner(cameraIdOrConfig) {
    logToScreen(
      "Memulai scanner dengan: " + JSON.stringify(cameraIdOrConfig),
      "info",
    );
    readerEl.innerHTML = "";
  ...
  function attemptCamera() {
    ...
    logToScreen(
      "Mencoba akses kamera belakang (environment) langsung…",
      "info",
    );
  ...
  function pickAndStart(preferredFacing) {
    Html5Qrcode.getCameras()
      .then(function (cameras) {
        if (!cameras || cameras.length === 0) {
          ...
          if (retry) {
            retry.addEventListener("click", function () {
              readerEl.innerHTML =
                '<div class="text-muted p-3">Mencoba ulang…</div>';
              attemptCamera();
            });
          }
          return;
        ...
        logToScreen(
          "Menggunakan kamera: " + (chosen.label || chosen.id),
          "info",
        );
    ...
    fileInput.addEventListener("change", function (e) {
      ...
      if (uploadResult)
        uploadResult.innerHTML =
          '<div class="text-muted small">Memproses gambar…</div>';
      ...
      html5Qr
        ...
        .catch(function (err) {
          logToScreen(
            "QR Code tidak terdeteksi pada gambar. Pastikan gambar jelas dan tidak terpotong.",
            "error",
          );
          if (uploadResult) {
            uploadResult.innerHTML =
